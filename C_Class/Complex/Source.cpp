...
void SetBox(char map[WIDTH][HEIGHT], Box* box)
{
	Box box[4]; //박스 좌표 초기화
	for (int i = 0; i < 4; i++)
	{
...
void MoveBox(char map[WIDTH][HEIGHT], Player* player, Box* box)
{
	char key = _getch();	// key 입력을 받아주는 함수
	if (key == ESC)
	{
		exit(0);
	}
	...
	switch (key)
	{
	case UP: if (map[player->y - 1][player->x / 2] == map[box]) { player->y--; }
		   break;
	case LEFT: if (map[player->y][player->x / 2 - 1] != '1') { player->x -= 2; }
			 break;
	case RIGHT: if (map[player->y][player->x / 2 + 1] != '1') { player->x += 2; }
